...
export default function App({ userData }) {
  ...
  const [anonymous, setAnonymous] = useState(userData?.identityMode === "Anonymous" ? true : false);
  ...
  const openCreatePost = () => {
    setNewPost({ text: "", topic: "Anxiety", anonymous });
  ...
  const createPost = () => {
    ...
    const post = {
      id: Date.now(),
      name: newPost.anonymous ? "Anonymous" : "Maya",
      time: "Just now",
      avatar: newPost.anonymous ? "avatar-purple" : "avatar-alex",
      text,
      tag: newPost.topic,
      tagClass: selectedTopic?.[1] || "lavender",
      likes: 0,
      comments: 0,
      liked: false,
      commentList: [],
    ...
    setAnonymous(newPost.anonymous);
    setShowCreatePost(false);
    setNewPost({ text: "", topic: "Anxiety", anonymous: true });
  };
  ...
  const addComment = (postId, text) => {
    ...
    setPosts((current) =>
      current.map((post) =>
        post.id === postId
          ? { ...post, comments: post.comments + 1, commentList: [...post.commentList, text.trim()] }
          : post
      )
    );
  };
  ...
  return (
    <div className={`app-shell ${active === "Journal" ? "no-right-panel" : ""}`}>
          ...
          <>
            ...
            <section className="sharing-card">
              <div>
                ...
                <p>You choose how you show up.</p>
              </div>
              <div className="sharing-options">
                <button className={anonymous ? "selected" : ""} onClick={() => setAnonymous(true)}>
                  <EyeOff size={15} />
                  <span><b>Anonymous</b><small>Share without name</small></span>
                </button>
                <button className={!anonymous ? "selected" : ""} onClick={() => setAnonymous(false)}>
                  <UserRound size={15} />
                  <span><b>Public</b><small>Share as yourself</small></span>
                </button>
              </div>
            </section>
              ...
              <div className="posts">
                {filteredPosts.length === 0 && (
                  <div className="empty-state">
                    ...
                    <span>Try searching for another topic or word.</span>
                  </div>
                )}
                  ...
                  <article className="post-card" key={post.id}>
                    ...
                    <div className="post-body">
                      ...
                      <div className="post-actions">
                        ...
                        <button className="share-post" onClick={() => setShowShare(post.id)}>
                          <Share2 size={14} /> Share
                        </button>
                      </div>
                    </div>
                    <button className="more" onClick={() => setShowPostMenu(showPostMenu === post.id ? null : post.id)}>
                      <MoreHorizontal size={18} />
                    </button>
                    {showPostMenu === post.id && (
                      <div className="post-menu">
                        <button onClick={() => copyPost(post.text)}>
            ...
            <section className="stories-card">
              <div className="stories-heading">
                <div>
                  <h2>Real stories. Real people. <Sparkles size={13} /></h2>
                  <p>Read testimonies from people who've been there.</p>
                </div>
                ...
                {stories.map((story, index) => (
                  <div className="story" key={index}>
                    ...
                    <div className="story-copy">
                      <b>{story.title}</b>
                      <small><span className="tiny-avatar" />{story.person}</small>
      ...
      {active !== "Journal" && (
        <aside className="right-panel">
          <div className="right-search">
            <ArrowUpDown size={14} />
            <input placeholder="Search Inside Out..." />
            <Search size={15} />
          </div>
          ...
          <section className="checkin card">
            ...
            <div className="moods">
              {[["Good", Smile], ["Okay", Smile], ["Not great", Meh], ["Struggling", Smile], ["Awful", Frown]].map(([label, Icon], index) => (
                <button key={label} className={mood === label ? "mood-active" : ""} onClick={() => setMood(label)}>
                  <span className={`mood-circle m${index}`}><Icon size={20} /></span>
                  <small>{label}</small>
                </button>
              ))}
            </div>
          ...
          <section className="reminder card">
            <div className="reminder-copy">
              ...
              <p>You don't have to have it all figured out. Just take it one day at a time.</p>
            </div>
            <div className="plant">
              <div className="pot" />
              <div className="stem" />
          ...
          <section className="trending card">
            ...
            <div className="topic-list">
              {topics.map(([name, color]) => (
                <span className={`topic ${color}`} key={name}>{name}</span>
              ))}
            </div>
          </section>
        </aside>
          ...
          <div className="modal create-modal">
            <div className="modal-header">
              <div>
                ...
                <p>This is a safe space. Say what's on your mind.</p>
              </div>
              <button className="modal-close" onClick={() => setShowCreatePost(false)}><X size={19} /></button>
            </div>
            <div className="post-mode">
              <button className={newPost.anonymous ? "mode-selected" : ""} onClick={() => setNewPost({ ...newPost, anonymous: true })}>
                <EyeOff size={17} />
                <span><b>Anonymous</b><small>Nobody will see your name.</small></span>
              </button>
              <button className={!newPost.anonymous ? "mode-selected" : ""} onClick={() => setNewPost({ ...newPost, anonymous: false })}>
                <UserRound size={17} />
                <span><b>Public</b><small>Post using your profile.</small></span>
              </button>
            </div>
            <textarea
            ...
            <div className="topic-selector">
              <label>Topic</label>
              <select value={newPost.topic} onChange={(e) => setNewPost({ ...newPost, topic: e.target.value })}>
            ...
            <div className="modal-footer">
              ...
              <button className="publish-btn" disabled={!newPost.text.trim()} onClick={createPost}>
                <Send size={15} /> Post
              </button>
            </div>
          </div>
      ...
      {showComments !== null && (
        <CommentsModal
          post={posts.find((post) => post.id === showComments)}
      ...
      {showShare !== null && (
        <div className="modal-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) setShowShare(null); }}>
          <div className="modal small-modal">
            <div className="modal-header">
              <div><h2>Share post</h2><p>Share this thought with someone.</p></div>
              <button className="modal-close" onClick={() => setShowShare(null)}><X size={19} /></button>
            </div>
            <div className="share-options">
              <button onClick={() => copyPost(posts.find((p) => p.id === showShare)?.text || "")}>
                <Copy size={18} /><span>Copy text</span>
              </button>
